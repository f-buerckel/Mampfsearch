...
RESULTS_DIR = "/home/fbuerckel/Mampfsearch/Mampfsearch/src/mampfsearch/evaluation/Results"

# Output directory for aggregated results (will be created if not exists)
# Defaults to a timestamped folder inside RESULTS_DIR
OUTPUT_DIR = os.path.join(RESULTS_DIR, f"aggregated_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
def aggregate_data(results_dir: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    ...
    df_evaluated = pd.concat(evaluated_list, ignore_index=True) if evaluated_list else pd.DataFrame()
    df_pairwise = pd.concat(pairwise_list, ignore_index=True) if pairwise_list else pd.DataFrame()
    df_gen_multi = pd.concat(gen_multi_list, ignore_index=True) if gen_multi_list else pd.DataFrame()
    df_gen_unstructured = pd.concat(gen_unstructured_list, ignore_index=True) if gen_unstructured_list else pd.DataFrame()
...
def calculate_statistics(df_evaluated: pd.DataFrame, df_pairwise: pd.DataFrame) -> str:
    ...
    if not df_evaluated.empty:
        ...
        if "lecture_name" in df_evaluated.columns:
            ...
            lecture_means = df_evaluated.groupby(["lecture_name", "Method"])[numeric_cols].mean().reset_index()
            ...
            for method in df_evaluated["Method"].unique():
                ...
                target_col = "overall_review_score" if "overall_review_score" in numeric_cols else numeric_cols[0]
                ...
                for _, row in method_lecture_means.iterrows():
                    ...
                    if abs(diff) > 1.0: # Arbitrary threshold for "very different"
                        stats_output.append(f"  [OUTLIER] {lec}: Score {val:.2f} (Diff: {diff:+.2f})")
    ...
    if not df_pairwise.empty and "winner" in df_pairwise.columns:
        ...
        if "Run" in df_pairwise.columns:
            ...
            global_multi_win_rate = len(df_pairwise[df_pairwise["winner_clean"] == "Multi-Entity"]) / total_pairs if total_pairs > 0 else 0
            stats_output.append(f"Global Multi-Entity Win Rate: {global_multi_win_rate:.2%}")
            
            for run_name, group in run_groups:
                n = len(group)
                if n < 5: continue # Skip small samples
                ...
                if abs(diff) > 0.20:
                    stats_output.append(f"  [OUTLIER] {run_name}: Multi-Entity Win Rate {win_rate:.2%} (Diff: {diff:+.2%})")
...
def generate_pairwise_win_rate_plot(df: pd.DataFrame, output_dir: str):
    ...
    if "Run" in df.columns:
        ...
        ct.plot(kind='barh', stacked=True, colormap='coolwarm', edgecolor='black', figsize=(10, max(6, len(ct) * 0.4)))
...
def main():
    ...
    if not df_eval.empty and "lecture_name" in df_eval.columns:
        ...
        if numeric_cols:
            df_lecture = df_eval.groupby(["lecture_name", "Method"])[numeric_cols].mean().reset_index()
            df_lecture.to_csv(os.path.join(OUTPUT_DIR, "aggregated_by_lecture.csv"), index=False)
    ...
    if not df_gen_multi.empty:
        df_gen_multi.to_csv(os.path.join(OUTPUT_DIR, "aggregated_generated_multi.csv"), index=False)
    if not df_gen_unstructured.empty:
        df_gen_unstructured.to_csv(os.path.join(OUTPUT_DIR, "aggregated_generated_unstructured.csv"), index=False)
    ...
    try:
        ...
        plot_radar_chart(df_eval, OUTPUT_DIR, "Aggregated QG", "Aggregated Eval")
        plot_box_chart(df_eval, OUTPUT_DIR, "Aggregated QG", "Aggregated Eval")
        plot_problem_criteria(df_eval, OUTPUT_DIR, "Aggregated QG", "Aggregated Eval")
