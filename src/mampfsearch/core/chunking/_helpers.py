...
def merge_until_min_size(
    subtitles: List[srt.Subtitle], min_size: int, overlap: bool
) -> List[srt.Subtitle]:
    """
...
def split_at_word_boundary(subtitle: srt.Subtitle, max_size: int) -> List[srt.Subtitle]:
    ...
    first_half = srt.Subtitle(
        index=subtitle.index, start=subtitle.start, end=split_time, content=first_text
    )

    second_half = srt.Subtitle(
        index=subtitle.index, start=split_time, end=subtitle.end, content=second_text
    )
...
def split_large_chunks(
    subtitles: List[srt.Subtitle], max_size: int
) -> List[srt.Subtitle]:
