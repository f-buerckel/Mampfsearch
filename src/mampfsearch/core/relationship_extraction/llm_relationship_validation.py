...
class RelationshipValidationLLM:
    ...
    def validate_relationship(self, rel: Relationship) -> bool:
        ...
        try:
            ...
            if "yes" in content:
                logger.debug(
                    f"Relationship '{rel.entity_1.text}' --[{rel.relationship}]--> '{rel.entity_2.text}' validated by LLM."
                )
                return True
            else:
                logger.info(
                    f"Relationship '{rel.entity_1.text}' --[{rel.relationship}]--> '{rel.entity_2.text}' rejected by LLM as context-specific."
                )
                return False
